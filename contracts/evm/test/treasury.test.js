...
describe("AgentTreasury — Agent 经济资金池", function () {
  let token, treasury, owner, agent, buyer, snapshotId;
  ...
  beforeEach(async function () {
    ...
    treasury = await Treasury.deploy(await token.getAddress(), ethers.parseUnits("100", 6)); // dailyLimit=$100
  ...
  it("payAgent 信誉不足拒绝", async function () {
    ...
    await expect(treasury.payAgent(agent.address, ethers.parseUnits("10", 6)))
      .to.be.revertedWith("reputation too low");
  });

  it("payAgent 日预算超限拒绝 (状态 dailyLimit, 不可绕过)", async function () {
    await treasury.deposit(ethers.parseUnits("1000", 6));
    // dailyLimit 默认 $100 → 累计超过即拒
    await treasury.payAgent(agent.address, ethers.parseUnits("60", 6));
    await expect(treasury.payAgent(agent.address, ethers.parseUnits("60", 6)))
      .to.be.revertedWith("daily limit exceeded");
  });

  it("payAgent 成功支付 + 日支出记录", async function () {
    await treasury.deposit(ethers.parseUnits("1000", 6));
    await treasury.payAgent(agent.address, ethers.parseUnits("10", 6));
  ...
  it("frozen 后拒绝所有操作", async function () {
    ...
    await expect(treasury.payAgent(agent.address, ethers.parseUnits("1", 6)))
      .to.be.revertedWith("treasury frozen");
  });

  it("updateAllocation 权重校验 (非 1000 拒绝)", async function () {
    await expect(treasury.updateAllocation([100, 100, 100, 100, 100, 100]))
      .to.be.revertedWith("weights != 1000");
    await treasury.updateAllocation([500, 200, 100, 100, 50, 50]);
    const a = await treasury.allocation();
    expect(a.agentRewards).to.equal(500);
  });

  it("两段式所有权转移", async function () {
    await treasury.transferOwnership(buyer.address);
    await expect(treasury.connect(buyer).acceptOwnership()).to.emit(treasury, "OwnershipTransferred");
    expect(await treasury.owner()).to.equal(buyer.address);
  });

  it("紧急提款需等待期", async function () {
    await treasury.deposit(ethers.parseUnits("1000", 6));
    await treasury.requestEmergencyWithdraw();
    await expect(treasury.emergencyWithdraw(buyer.address))
      .to.be.revertedWith("delay not elapsed");
    // 快进 2 天
    await ethers.provider.send("evm_increaseTime", [2 * 86400]);
    await ethers.provider.send("evm_mine", []);
    await treasury.emergencyWithdraw(buyer.address);
    expect(await token.balanceOf(buyer.address)).to.equal(ethers.parseUnits("1000", 6));
  });
});
