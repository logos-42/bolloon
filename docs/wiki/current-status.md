...
|| **UI 修复批: 斜杠命令/用户名编辑/发送配置/中断/渲染去重** (2026-08-02) | `src/web/client.ts` + `src/web/index.html` + `src/web/ui/message-renderer.ts` + `src/web/style.css` | ① / 斜杠命令菜单 (plan/todo/review/task/goal/skill/add-friend/help) Enter/Tab 插入输入框 + server 路由; ② 用户名内联编辑 PUT /api/user/identity; ③ 发送工具开关 🔧 (仅远端 P2P 显示, per-message autoInvokeTools 透传 chat-send); ④ abort 后立即广播 done; ⑤ 回复重复渲染修复: loadSession 后 seedDedupState (lastAiContent 未更新导致 SSE resume 补包重复渲染); ⑥ judgments 徽标溢出修复; ⑦ 远端 channel 🗑️ 删除 + localStorage ignore 集合 (对端再广播被过滤); ⑧ P2P 对话框点外部关闭 + 远端对话本地缓存 (localStorage 按 peer+channel, 打开先读本地再静默合并远程). tsc 0 错, vitest 993/993, build 全绿 | [client.ts](../../src/web/client.ts) |
