...
- **真实输出(全部真跑, 非自述)**: `npx tsx scripts/verify-chain-bridge.ts` → **45 passed / 0 failed**, 含真 txHash(`createEscrowV2 0xaeb9550b…` block 96 / `submitProofV2 0xf95d4aa3…` block 97 / `releaseV2 0x225d2e14…` block 98 · escrow B `0x66923408…` block 101); **真重组**用 anvil `anvil_rollback [1]`(块 101 被回滚 → 判定 `reorged`, 且 escrow B 链上状态真回退 `RELEASED→ACTIVE`); 抬高门槛 `confirmed=500` → `pending`(不判已结算); 换错 taskKey/事件名 → `event_mismatch`; 掉 RPC(`127.0.0.1:9`) → `unknown`+`rpcAvailable:false`。单测 `src/test/chain-{config,escrow-client,settlement-verifier,wallet-authorization,state-store,paid-info-f5}.test.ts` **112/112** · `npx tsc --noEmit` **0 错**。
