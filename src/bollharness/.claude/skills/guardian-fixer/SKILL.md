...
```bash
# 从 worktree 运行，用主仓库的测试
cd /tmp/{{PROJECT_NAME}}-$ISSUE_SLUG
npx ts-node backend/tests/unit/           # 无 Docker
npx ts-node backend/tests/field/          # 无 Docker
npx ts-node backend/tests/test_phase0_surface.ts  # 无 Docker
npx ts-node backend/tests/product/        # 需要 Docker
npx ts-node backend/tests/matching/      # 无 Docker
```
