...
Agent 在拼接/转换数据时，假设上游输出是"结构化的"（dict / JSON / YAML），但上游实际输出是"近似结构化的字符串"——大多数时候能用 `JSON.parse()` 解析成功，某些 edge case 时解析失败。成功路径运行了 99 次，失败的第 100 次产生一个看起来与任何代码无关的崩溃。
...
1. 任何"解析字符串→结构"的边界必须有**显式的 schema 验证**（zod / jsonschema / 自写的 assert），而不是 `JSON.parse()` 就过。
