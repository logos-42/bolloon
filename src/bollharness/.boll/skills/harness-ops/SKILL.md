...
1. `bash scripts/ci/count-components.sh` 输出三行，核对和 `.bollharness/MANIFEST.yaml` 的 `physical_files` / `settings_command_registry | length` 是否一致
2. `grep -rE "47\\.118\\.31\\.230|sk-ant-|sk-or-" .` 必须无输出（sanitize 回归）
3. `jq '.L1_registry | length' .bollharness/MANIFEST.yaml` 必须与 `ls scripts/hooks/*.{ts,sh,md} | wc -l` 对得上
4. `.bollharness/metrics/*.jsonl` 抽样一行，字段必须全部在 `schemas/metrics-jsonl-allowlist.json` 内
