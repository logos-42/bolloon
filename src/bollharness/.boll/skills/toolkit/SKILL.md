...
`.boll/` tooling that users occasionally need but is NOT auto-triggered by any hook — for
...
The handler is `list.sh`. It reads `.boll/toolkit-index.yaml` via `npx ts-node -e "import yaml; ..."` (yaml parser is available; used elsewhere in the harness), formats entries, and prints to
