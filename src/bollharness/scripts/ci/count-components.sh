...
#   command_instances=<N>   # count of "command": entries in .boll/settings.json
...
#
# Run from repo root. Reads .boll/settings.json, scripts/hooks/, scripts/.
...
if command -v jq >/dev/null 2>&1; then
  ...
  ] | length' .boll/settings.json)
else
  # Fallback: line-grep. Less precise but works without jq.
  CMD_INSTANCES=$(grep -c '"command":' .boll/settings.json || echo 0)
fi

# unique_scripts: distinct scripts referenced by those commands.
# Extract the last `npx ts-node scripts/...` or `bash scripts/...` token and dedupe.
UNIQUE_SCRIPTS=$(grep -oE '(npx ts-node|bash) scripts/[^ "]+' .boll/settings.json \
  | awk '{print $2}' | sort -u | wc -l | tr -d ' ')

# physical_files: hooks dir + 2 root scripts
HOOKS_COUNT=$(ls scripts/hooks/*.ts scripts/hooks/*.sh scripts/hooks/*.md 2>/dev/null | wc -l | tr -d ' ')
ROOT_COUNT=$(ls scripts/guard-feedback.ts scripts/deploy-guard.ts 2>/dev/null | wc -l | tr -d ' ')
