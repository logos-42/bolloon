#!/bin/bash
# Walks up from $PWD to find the bollharness-installed project root.
#
# Anchor priority:
...
# guard-feedback.ts (because they disabled it via issue-adapter.yaml) would
...
d="${PWD}"
while [ "$d" != "/" ] && [ -n "$d" ]; do
  if [ -f "$d/.bollharness/MANIFEST.yaml" ]; then
    echo "$d"
    exit 0
  fi
  d="$(dirname "$d")"
