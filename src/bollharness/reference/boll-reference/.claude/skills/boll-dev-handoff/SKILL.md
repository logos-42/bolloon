---
name: boll-dev-handoff
description: 流形开发接手入口。用于新 AI 接手 boll 开发、梳理最新上下文、开始生产排障、继续上一轮工作或确认当前主线时使用。
status: active
tier: entry
owner: nature
...
这个 skill 不负责复述 boll 的全部历史，也不负责重新总结理念。

它只负责一件事：**让新接手 boll 开发的 AI，在一次入场中找到最新、可执行、可验证的上下文入口。**
...
每次新 AI 接手 boll 时，按这个顺序执行：

1. 先运行：
   ```bash
   python3 .claude/skills/boll-dev-handoff/scripts/collect_handoff_context.py
   ```
...
boll 有一套上下文工程系统（不是 prompt），通过 PostToolUse hook 在编辑文件时自动注入相关思维框架：
...
```bash
python3 .claude/skills/boll-dev-handoff/scripts/collect_handoff_context.py \
  --cloud-root ~/.claude/projects/-Users-nature------boll \
  --recent-sessions 3 \
  --recent-messages 3
```
