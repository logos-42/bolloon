...
export class ConfigModal extends HTMLElement {
    ...
    show(data) {
        ...
        const apiKeyEl = this.shadow.getElementById('api-key');
        const baseUrlEl = this.shadow.getElementById('base-url');
        const modelEl = this.shadow.getElementById('model');
        const tempEl = this.shadow.getElementById('temperature');
        const testResultEl = this.shadow.getElementById('test-result');
        if (titleEl)
            titleEl.textContent = `配置 ${data.info.name}`;
        if (apiKeyEl)
            apiKeyEl.value = (data.provider.apiKey || '').replace(/\*\*\*/g, '');
        if (baseUrlEl)
            baseUrlEl.value = data.provider.baseUrl || '';
        if (modelEl)
            modelEl.value = data.provider.model || '';
        if (tempEl)
            tempEl.value = String(data.provider.temperature || 0.7);
        if (testResultEl) {
